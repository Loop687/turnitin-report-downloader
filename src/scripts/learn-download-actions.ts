...
async function navigateToFinalAIPage(scraper: ImprovedTurnitinScraperService, page: Page, session: DownloadSession): Promise<void> {
    const rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout
    });
...
async function setupActionListeners(page: Page, session: DownloadSession): Promise<void> {
    ...
    await page.evaluateOnNewDocument(() => {
        function generateXPath(element: Element): string {
            ...
            while (current && current.parentElement) {
                ...
                if (parts.length > 8) break; // Limitar profundidad
            }
    ...
    await page.evaluateOnNewDocument(() => {
        document.addEventListener('click', (event) => {
            ...
            if (target) {
                ...
                (window as any).capturedActions = (window as any).capturedActions || [];
                (window as any).capturedActions.push(actionData);
            }
        }, true);
    });
    ...
    const actionMonitor = setInterval(async () => {
        try {
            const capturedActions = await page.evaluate(() => {
                const actions = (window as any).capturedActions || [];
                (window as any).capturedActions = []; // Limpiar después de obtener
                return actions;
            });
            
            // Agregar acciones capturadas a la sesión
            capturedActions.forEach((action: UserAction) => {
...
async function generateAutomaticScript(session: DownloadSession, downloadPath: string): Promise<void> {
    const scriptContent = `
