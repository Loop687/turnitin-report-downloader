...
const EXACT_JSON_DATA = {
    // This CSS selector is used to find the AI detection percentage button 
    // on the Turnitin document viewer page (Carta - ev.turnitin.com/app/carta).
    // Clicking this button typically opens the AI writing report panel or a new tab/page.
...
// 🔥 NUEVO: Variables globales para mantener la sesión del navegador
let globalScraperInstance: ImprovedTurnitinScraperService | null = null;
let globalLoggedInPage: Page | null = null;
let isSessionActive = false;

// 🔥 NUEVA FUNCIÓN: Obtener o crear instancia del scraper
async function getOrCreateScraperSession(): Promise<{scraper: ImprovedTurnitinScraperService, inboxPage: Page}> {
    if (globalScraperInstance && globalLoggedInPage && isSessionActive) {
        ...
        try {
            ...
            // Navegar de vuelta a la bandeja de entrada si no estamos ahí
            if (!globalLoggedInPage.url().includes('inbox')) {
                console.log('🔄 Navegando de vuelta a la bandeja de entrada...');
                // Ensure navigation targets the generic inbox URL for manual navigation if needed.
                // The actual specific inbox URL will be navigated to by the user.
            ...
            // La sesión no es válida, crear nueva
    ...
    const scraper = new ImprovedTurnitinScraperService(true); // Headless = true
...
export async function coordinateBasedDownloader(invokedTargetWorkTitle?: string, invokedSubmissionId?: string): Promise<DownloadResult> {
    ...
    try {
        ...
        console.log('===================================');
        // console.log('Estrategia: Uso de Submission ID para mayor precisión'); // Updated below
        console.log('');

        // 🔥 CAMBIO PRINCIPAL: Usar sesión reutilizable
        ...
        // 🔥 MODIFICADO: Priorizar Submission ID, luego título
        if (!submissionId && !targetWorkTitle) { 
            const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
            const askQuestion = (question: string): Promise<string> => new Promise((resolve) => rl.question(question, resolve));
        ...
        // 🔥 NUEVO: Cerrar ventanas de documentos anteriores antes de abrir el nuevo
        ...
        // Usar el searchCriteria (ID o título) para detectar el archivo.
        // Si se usó ID para buscar, el nombre del archivo idealmente debería reflejarlo o ser genérico.
        // Si se usó título para buscar, esto es más directo.
        let downloadedFilePath: string | null;
        
        if (searchType === 'Submission ID') {
            // Para Submission ID, buscar archivos más genéricamente ya que el nombre puede ser diferente
            downloadedFilePath = await detectAndConfirmDownloadById(projectDownloadPath, searchCriteria);
        } else {
            // Para título, usar la detección original
        ...
        if (downloadedFilePath) {
            ...
            // 🔥 IMPORTANTE: Navegar de vuelta a la bandeja de entrada para futuras descargas
            ...
            // 🔄 Navegar de vuelta a la bandeja de entrada incluso si falla
            try {
                await scraper.navigateToTurnitinInbox(globalLoggedInPage!);
            } catch (navError) {
        ...
        // En caso de error crítico, intentar navegar de vuelta a la bandeja de entrada
        if (globalLoggedInPage && isSessionActive) {
            try {
                ...
                // Si no podemos navegar de vuelta, invalidar la sesión
                isSessionActive = false;
            }
        }
        ...
        // 🔥 CAMBIO CRÍTICO: NO cerrar el navegador ni invalidar la sesión aquí
        console.log('ℹ️ Navegador y sesión mantenidos para reutilización.');
    }
}

// 🔥 NUEVA FUNCIÓN: Para cerrar manualmente la sesión global
...
// 🔥 NUEVA FUNCIÓN: Para cerrar ventanas de documentos existentes
async function closeExistingDocumentWindows(mainPage: Page): Promise<void> {
    ...
    for (const page of allPages) {
        if (page === mainPage || page === globalLoggedInPage) {
            // No cerrar la página principal de la bandeja de entrada
            continue;
        }

        const url = page.url();
        // Identificar URLs de visores de documentos de Turnitin
...
async function navigateToInboxAndLogin(scraper: ImprovedTurnitinScraperService, page: Page, rlInstance: readline.Interface): Promise<Page | null> {
    const askLoginQuestion = (question: string): Promise<string> => new Promise((resolve) => rlInstance.question(question, resolve));
    
    try {
        console.log('🌐 Iniciando proceso de login y navegación...');
        await scraper.navigateToTurnitinInbox(page); // This navigates to turnitin.com and gives instructions
        
        // User manually logs in and navigates to the specific class inbox
        console.log('');
        ...
        // NO cerrar rlInstance aquí. Se maneja en el 'finally' de coordinateBasedDownloader.
    }
}

async function findWorkAndOpenAIReport(scraper: ImprovedTurnitinScraperService, page: Page, searchCriteria: string, searchType: string, projectDownloadPath: string): Promise<Page | null> {
    try {
        ...
        if (searchType === 'Submission ID') {
            ...
            // Asumimos que implementarás este método en ImprovedTurnitinScraperService
            // Este método debe buscar el submissionId como texto y hacer clic en el elemento apropiado (ej. el título del trabajo en la misma fila)
            foundAndClicked = await scraper.findAndClickOnSubmissionById(page, searchCriteria);
        } else { // searchType === 'título'
        ...
        // 🔥 VALIDACIÓN ADICIONAL: Verificar que el Submission ID en la URL coincida si se buscó por ID
        ...
        if (urlSubmissionId) {
            ...
            if (searchType === 'Submission ID' && urlSubmissionId !== searchCriteria) {
                ...
                // Considerar si se debe abortar o continuar con advertencia. Por ahora, continuar.
        ...
        if (!aiButton) {
            ...
            try { await cartaPage.screenshot({ path: debugPath }); console.log(`📸 Screenshot de debug (sin botón IA) guardado en ${debugPath}`); } catch (e) {}
            throw new Error('No se encontró el botón de IA en la página de Carta.');
        }

        const newPagePromiseAfterAIClick = new Promise<Page | null>(resolve => {
        ...
        if (!aiReportTargetPage) {
            ...
            if (cartaPage.url().includes('integrity.turnitin.com')) {
                ...
                if (integrityPage) {
                    ...
                    try { await cartaPage.screenshot({ path: debugPath }); console.log(`📸 Screenshot de debug (sin pág. reporte IA) guardado en ${debugPath}`); } catch (e) {}
...
async function waitForCompletePageLoad(page: Page): Promise<void> {
    console.log('\n⏳ ESPERANDO CARGA COMPLETA DE LA PÁGINA - VERSIÓN OPTIMIZADA');
    console.log('=============================================================');

    // 🔥 OPTIMIZADO: Reducir intentos de red y tiempo de espera
    console.log('📡 Esperando estabilización de red (optimizado)...');
    for (let i = 0; i < 2; i++) { // Reducido de 3 a 2 intentos
        try {
            await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 10000 }); // Cambiado a networkidle2 y reducido timeout
            console.log(`✅ Red estable en intento ${i + 1}`);
            break;
        } catch (error) {
            console.log(`⚠️ Intento ${i + 1} de red falló, continuando...`);
            await page.waitForTimeout(3000); // Reducido de 10s a 3s
        }
    }
    
    // 🔥 OPTIMIZADO: Espera base más corta
    console.log('⏱️ Espera base (10 segundos)...');
    await page.waitForTimeout(10000); // Reducido de 30s a 10s
    
    // 🔥 OPTIMIZADO: Configuración de viewport más rápida
    ...
    // 🔥 OPTIMIZADO: Verificación de elementos más rápida
    console.log('🎯 Verificando elementos clave...');
    const elementsToWaitFor = [
        'body',
        'tii-ai-writing-app'
    ]; // Reducido a elementos esenciales
    
    for (const selector of elementsToWaitFor) {
        try {
            await page.waitForSelector(selector, { visible: true, timeout: 5000 }); // Reducido de 30s a 5s
    ...
    // 🔥 OPTIMIZADO: Espera final más corta
    console.log('⏱️ Espera final para renderizado (15 segundos)...');
    await page.waitForTimeout(15000); // Reducido de 45s a 15s
    ...
    try {
        ...
        if (elementCount < 50) { // Reducido umbral de 100 a 50
...
async function visualDownloadStrategy(page: Page, projectDownloadPath: string): Promise<void> {
    console.log('\n🎨 ESTRATEGIA VISUAL DE DESCARGA OPTIMIZADA');
    console.log('===========================================');
    
    // 🔥 OPTIMIZADO: Verificación de viewport más rápida
    ...
    try {
        ...
        await page.waitForTimeout(2000); // Reducido de 4s a 2s
        ...
        // 🔥 OPTIMIZADO: Coordenadas del botón AI Writing Report
        ...
        await page.waitForTimeout(3000); // Reducido de 7s a 3s
        ...
        await page.waitForTimeout(8000); // Reducido de 15s a 8s
        return; 
...
async function detectAndConfirmDownload(projectDownloadPath: string, targetWorkTitle: string): Promise<string | null> {
    ...
    // 🔥 OPTIMIZADO: Patrones de búsqueda más eficientes
    ...
    ]; // Reducido número de patrones
    
    console.log(`   📋 Buscando: ${searchPatterns.slice(0, 2).join(', ')}...`); // Solo mostrar primeros 2

    // 🔥 OPTIMIZADO: Menos intentos pero más frecuentes
    const totalAttempts = 6; // Aumentado de 5 a 6
    const waitTimeBetweenAttempts = 3000; // Reducido de 6s a 3s
            ...
            try {
                ...
                for (const file of files) {
                    ...
                    try {
                        ...
                        const isRecent = (Date.now() - stats.mtimeMs) < (5 * 60 * 1000); // Reducido de 10min a 5min
                        
                        if (isRecent) {
                            // 🔥 OPTIMIZADO: Verificación más rápida de patrones
                            ...
                            // 🔥 OPTIMIZADO: Búsqueda por palabras más rápida
                        ...
                        // Continuar sin logging para ser más rápido
                    }
                }
    ...
    for (const location of downloadLocations) {
        try {
            ...
            }).slice(0, 3); // Solo mostrar primeros 3
...
// 🔥 NUEVA FUNCIÓN: Detectar descarga usando Submission ID (más flexible)
async function detectAndConfirmDownloadById(projectDownloadPath: string, submissionId: string): Promise<string | null> {
    ...
    // Para Submission ID, buscar cualquier PDF reciente ya que el nombre puede ser genérico
    ...
    for (let attempt = 1; attempt <= totalAttempts; attempt++) {
        ...
        for (const location of downloadLocations) {
            try {
                ...
                let mostRecentPdf: { file: string, filePath: string, mtime: number } | null = null;
                ...
                for (const file of files) {
                    ...
                    try {
                        const filePath = path.join(location, file);
                        const stats = fs.statSync(filePath);
                        const isRecent = (Date.now() - stats.mtimeMs) < (5 * 60 * 1000); // 5 minutos
...
if (require.main === module) {
    coordinateBasedDownloader().then(result => {
        ...
        console.log('---------------------------------');
        // El readline se cierra dentro de coordinateBasedDownloader
        // y el navegador también. No es necesario preguntar para cerrar aquí.
        // process.exit(result.success ? 0 : 1); // Opcional: salir con código de estado
    }).catch(error => {
        console.error('\n--- ERROR INESPERADO EJECUTANDO SCRIPT ---');
        console.error(error);
        console.error('------------------------------------------');
        // process.exit(1); // Opcional: salir con código de estado
    });
}
